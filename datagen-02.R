...
for(i in 1:N_participants){
  ...
  # These are variables which have some relation to the probability of missingness
  Z1_now <- runif(n = tot_decision_points, min = -0.8, max = -0.3)
  
  # Calculate probability of missingness: MCAR
  dat_observed <- dat_observed %>% 
    # Note that treatment indicator is NA if I_now=0
    # this will affect which values of M_now are NA as well
    # Note: if treatment indicator is NA, we require a placeholder value for M_now
    mutate(prob_M_now = if_else(is.na(A_now), 
                                0, # sanity check: change this value to any real number between 0 and 1
                                exp(-0.7 + 1*Z1_now + A_now*(0.1 + 0.1*Z1_now))
                                ))
...
save(dat_all, list_datagen_params, file = "simulated_datasets/simdat02.RData")
